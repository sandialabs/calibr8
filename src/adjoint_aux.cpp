...
static int get_num_aux_dofs(RCP<State> state) {
  int const model_form = state->model_form;
  RCP<LocalResidual<double>> local = state->residuals->local[model_form];
  int ndofs = 0;
  for (int i = 0; i < local->num_aux_vars(); ++i) {
    ndofs += local->aux_var_num_eqs(i);
  }
  return ndofs;
}

AdjointAux::AdjointAux(RCP<ParameterList> params, RCP<State> state, RCP<Disc> disc) {
  m_params = params;
  m_state = state;
  m_disc = disc;
  (void)params;
...
void AdjointAux::initialize_history_vectors() {
...
void AdjointAux::solve_and_compute_error_at_step(
  ...
  solve_adjoint_aux_local_and_estimate_error(
      m_state,
      m_disc,
      aux_history,
      local_history,
