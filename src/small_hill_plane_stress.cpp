...
static ParameterList get_valid_local_residual_params() {
  ParameterList p;
  p.set<std::string>("type", "small_hill_plane_stress");
...
template <typename T>
SmallHillPlaneStress<T>::SmallHillPlaneStress(ParameterList const& inputs, int ndims) {
...
template <typename T>
SmallHillPlaneStress<T>::~SmallHillPlaneStress() {
}

template <typename T>
void SmallHillPlaneStress<T>::init_params() {
...
template <typename T>
void SmallHillPlaneStress<T>::init_variables_impl() {
...
int SmallHillPlaneStress<FADT>::solve_nonlinear(RCP<GlobalResidual<FADT>> global, int step) {
  ...
  if ((iter > m_max_iters) && (!converged)) {
    fail("SmallHillPlaneStress:solve_nonlinear failed in %d iterations", m_max_iters);
  }
...
int SmallHillPlaneStress<T>::evaluate(
  ...
  Tensor<T> const sigma_2D = this->cauchy(global);
  Tensor<T> sigma_3D = insert_2D_tensor_into_3D(sigma_2D);

  T const hill = compute_hill_value(sigma_3D, hill_params);
  ...
  if (!force_path) {
    ...
    if (f > m_abs_tol || std::abs(f) < m_abs_tol) {
      Tensor<T> const n_3D = compute_hill_normal(sigma_3D, hill_params, hill);
...
template <typename T>
Tensor<T> SmallHillPlaneStress<T>::cauchy(RCP<GlobalResidual<T>> global) {
  ...
  Tensor<T> const pstrain = this->sym_tensor_xi(0);
  T const epsilon_zz = this->epsilon_zz(global);
  T const epsilon_kk = trace(epsilon) + epsilon_zz;
  Tensor<T> const sigma = lambda * epsilon_kk * I
      + 2. * mu * (epsilon - pstrain);
  return sigma;
}

template <typename T>
Tensor<T> SmallHillPlaneStress<T>::dev_cauchy(RCP<GlobalResidual<T>> global) {
  int const ndims = global->num_dims();
  Tensor<T> const I = minitensor::eye<T>(ndims);
  Tensor<T> cauchy = this->cauchy(global);
  T hydro_cauchy = this->hydro_cauchy(global);
  return cauchy - hydro_cauchy * I;
}

template <typename T>
T SmallHillPlaneStress<T>::hydro_cauchy(RCP<GlobalResidual<T>> global) {
  Tensor<T> cauchy = this->cauchy(global);
  return trace(cauchy) / 3.;
}

template <typename T>
T SmallHillPlaneStress<T>::pressure_scale_factor() { return 0.; }

template <typename T>
T SmallHillPlaneStress<T>::epsilon_zz(RCP<GlobalResidual<T>> global) {
  T const E = this->m_params[0];
  T const nu = this->m_params[1];
  T const mu = compute_mu(E, nu);
  T const lambda = compute_lambda(E, nu);
