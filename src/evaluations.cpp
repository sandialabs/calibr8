...
Array1D<double> eval_qoi_gradient(RCP<State> state, int step) {
  ...
  int const num_embedded_params = state->residuals->local[model_form]->num_embedded_params();
  int const num_params = num_active_params + num_embedded_params;
  Array1D<double> grad(num_params);
  EVector Egrad = EVector::Zero(num_active_params);
  EVector Egrad_embedded = EVector::Zero(num_embedded_params);
        ...
        for (int pt = 0; pt < npts; ++pt) {
          ...
          if (ip_set == 0) {
            ...
            if (num_embedded_params > 0) {
              ...
              EMatrix const dC_dpT_embedded = dfad_local->eigen_jacobian(num_embedded_params).transpose();
              Egrad_embedded += dC_dpT_embedded * phi_pt;
            }
  ...
  // concatenate conventional and embedded parameter gradients
  EVector Egrad_total(num_params);
  Egrad_total << Egrad, Egrad_embedded;
