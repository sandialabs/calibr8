...
class Driver {
  ...
  private:
    ...
    void solve_adjoint_aux_and_compute_errors();
...
void Driver::solve_adjoint_aux_and_compute_errors() {
  print("SOLVING ADJOINT AND COMPUTING LOCAL ERRORS");
  auto disc = m_state->disc;
  m_adjoint_aux = rcp(new AdjointAux(m_params, m_state, disc));
  ...
  apf::Field* D_error = apf::createStepField(m, "D_error", apf::SCALAR);
  apf::zeroField(C_error);
  apf::zeroField(D_error);

  for (int step = nsteps; step > 0; --step) {
    m_adjoint_aux->solve_and_compute_error_at_step(step, C_error, D_error);
  }

  m_adjoint_aux = Teuchos::null;
}
...
void Driver::drive() {
  ...
  prepare_fine_space();
  solve_adjoint_aux_and_compute_errors();
