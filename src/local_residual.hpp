...
class LocalResidual {
  ...
  public:
    ...
    //! \brief Get the number of embedded model parameters
    int num_embedded_params() const { return m_num_embedded_params; }

    //! \brief Get the active optimization parameters
    Array1D<double> active_params() const {
      Array1D<double> active_params(m_num_active_params + m_num_embedded_params);
      int const num_elem_sets = m_elem_set_names.size();
      ...
      EVector embedded_params = get_embedded_params();
      for (int i = 0; i < m_num_embedded_params; ++i) {
        active_params[p] = embedded_params(i);
        ++p;
      }
    ...
    void set_params(Array1D<double> const& params) {
      ...
      EVector embedded_param_values(m_num_embedded_params);
      for (int i = 0; i < m_num_embedded_params; ++i) {
        embedded_param_values[i] = params[p];
        ++p;
      }
      set_embedded_params(embedded_param_values);
    }

    virtual EVector get_embedded_params() const {
      EVector empty;
      return empty;
    }

    virtual void set_embedded_params(EVector const&) {};
