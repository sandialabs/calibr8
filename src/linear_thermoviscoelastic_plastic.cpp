...
template <typename T>
void LTVEPlastic<T>::read_prony_series(ParameterList const& prony_files) {
...
template <typename T>
void LTVEPlastic<T>::compute_temperature(ParameterList const& inputs) {
...
template <typename T>
Array1D<double> LTVEPlastic<T>::compute_J3_k(
    double const psi,
...
template <typename T>
double LTVEPlastic<T>::compute_J3(Array1D<double> const& J3_k) {
  int const num_terms = m_vol_prony.size();
  double J3 = 0.;
...
template <typename T>
void LTVEPlastic<T>::residual_and_deriv(
    double const psi,
    Array1D<double> const& J3_k_prev,
    double const temp,
    double& r_val,
...
template <typename T>
double LTVEPlastic<T>::lag_nonlinear_solve(
    double const psi,
    Array1D<double> const& J3_k_prev,
    double const temp,
    double const tol,
...
template <typename T>
void LTVEPlastic<T>::compute_shift_factors() {
...
template <typename T>
void LTVEPlastic<T>::compute_delta_strains(
...
template <>
EMatrix LTVEPlastic<double>::daux_dxT(RCP<GlobalResidual<double>> global, int step) {
...
template <>
EMatrix LTVEPlastic<FADT>::daux_dxT(RCP<GlobalResidual<FADT>> global, int step) {
  int const num_global_dofs = global->num_dofs();
  EMatrix daux_dxT = EMatrix::Zero(num_global_dofs, this->m_num_aux_dofs);
...
template <typename T>
EVector LTVEPlastic<T>::daux_dchi_prev_diag(int step) {
...
template <typename T>
EMatrix LTVEPlastic<T>::dlocal_dchi_prevT(int step) {
...
template <typename T>
EVector LTVEPlastic<T>::eigen_aux_residual(RCP<GlobalResidual<T>> global, int step) {
...
template <typename T>
LTVEPlastic<T>::LTVEPlastic(ParameterList const& inputs, int ndims) {
...
template <typename T>
LTVEPlastic<T>::~LTVEPlastic() {
}

template <typename T>
void LTVEPlastic<T>::init_params() {
  ...
  this->m_param_names[6] = "Y";
  this->m_param_names[7] = "S";
  this->m_param_names[8] = "D";
  ...
  for (int es = 0; es < num_elem_sets; ++es) {
    ...
    this->m_param_values[es][6] = material_params.get<double>("Y");
    this->m_param_values[es][7] = material_params.get<double>("S");
    this->m_param_values[es][8] = material_params.get<double>("D");
  }
...
template <>
int LTVEPlastic<double>::solve_nonlinear(RCP<GlobalResidual<double>>, int step) {
  return 0;
}

template <>
int LTVEPlastic<FADT>::solve_nonlinear(RCP<GlobalResidual<FADT>> global, int step) {
  ...
  while ((iter <= m_max_iters) && (!converged)) {
    ...
    if (iter == 1) {
      ...
      this->evaluate(global, true, path, step);
    }
  ...
  if ((iter > m_max_iters) && (!converged)) {
    fail("LTVEPlastic:solve_nonlinear failed in %d iterations", m_max_iters);
  }
...
int LTVEPlastic<T>::evaluate(
  ...
  T const Y = this->m_params[6];
  T const S = this->m_params[7];
  T const D = this->m_params[8];
  ...
  T R_eqps;
  T const eqps_old = this->scalar_xi_prev(1);
  T const eqps = this->scalar_xi(1);

  Tensor<T> const s = minitensor::dev(cauchy);
  T const s_mag = minitensor::norm(s);
  Tensor<T> const n = s / s_mag;
  T const sigma_yield = Y + S * (1. - std::exp(-D * eqps));
  T const f = (s_mag - sigma_yield) / val(mu_g);
  double const mu_diff_tol = 1e-10;

  if (!force_path) {
    // plastic step
    if (f > m_abs_tol || std::abs(f) < m_abs_tol) {
      ALWAYS_ASSERT(std::abs(mu_g - mu_inf) < mu_diff_tol);
      T const dgam = eqps - eqps_old;
      R_cauchy += 2. * mu_inf * dgam * n;
      R_eqps = f;
      path = PLASTIC;
    }
    // elastic step
    else {
      R_eqps = eqps - eqps_old;
      path = ELASTIC;
    }
  }
  ...
  this->set_scalar_R(1, R_eqps);

  return path;
}

template <typename T>
Tensor<T> LTVEPlastic<T>::dev_cauchy(RCP<GlobalResidual<T>> global) {
...
template <typename T>
T LTVEPlastic<T>::hydro_cauchy(RCP<GlobalResidual<T>> global) {
  Tensor<T> const local_cauchy = this->sym_tensor_xi(0);
  int const ndims = this->m_num_dims;
  Tensor<T> const I = minitensor::eye<T>(ndims);
  if (ndims == 3) {
...
template <typename T>
T LTVEPlastic<T>::pressure_scale_factor() {
  return this->m_params[0];
}

template <typename T>
Tensor<T> LTVEPlastic<T>::cauchy(RCP<GlobalResidual<T>> global) {
  if (m_mode == MIXED) {
...
template <typename T>
Tensor<T> LTVEPlastic<T>::cauchy_mixed(RCP<GlobalResidual<T>> global) {
...
template class LTVEPlastic<double>;
template class LTVEPlastic<FADT>;
