...
T HybridHyperJ2PlaneStress<T>::nn_hardening(T alpha) {
  ...
  zero_vec(0) = 0. * alpha;
  /*
  T first_part = m_neural_network->evaluate(input_vec)[0];
  T second_part = m_neural_network->evaluate(zero_vec)[0];
  print("first input = %e", val(input_vec(0)));
  print("second input = %e", val(zero_vec(0)));
  print("first part = %e", val(first_part));
  print("second part = %e", val(second_part));
  */
...
int HybridHyperJ2PlaneStress<FADT>::solve_nonlinear(RCP<GlobalResidual<FADT>> global) {
  ...
  if ((iter > m_max_iters) && (!converged)) {
    fail("HybridHyperJ2PlaneStress:solve_nonlinear failed in %d iterations", m_max_iters);
  }
