...
class EGM_Objective {
  public:
    EGM_Objective(std::string const& input_file, bool evaluate_gradient);
    void evaluate();
  private:
    ...
    int m_model_form = 0;
    RCP<EquilibriumGap> m_eq_gap;
    std::string m_load_in_file;
    Array1D<double> m_load_data;
};

EGM_Objective::EGM_Objective(std::string const& input_file, bool evaluate_gradient) {
  ...
  if (is_multiple) {
    fail("EGM not set up for multiple problems");
  ...
  m_eq_gap = rcp(new EquilibriumGap(m_params, m_state[0], m_state[0]->disc));
  ParameterList& inverse_params = m_params->sublist("inverse", true);
}

void EGM_Objective::setup_opt_params(ParameterList const& inverse_params) {
...
void EGM_Objective::evaluate() {

  ParameterList& inverse_params = m_params->sublist("inverse", true);
  double dt;
  double eq_gap;
  double scaled_eq_gap;
  Array1D<double> eq_gap_vec;
  ...
  for (int prob = 0; prob < m_num_problems; ++prob) {
    ...
    eq_gap_vec.resize(nsteps, 0.);
    double const total_time = m_state[prob]->disc->time(nsteps) - m_state[prob]->disc->time(0);

    m_state[prob]->disc->destroy_primal();

      for (int step = 1; step <= nsteps; ++step) {
        eq_gap_vec[step - 1] = m_eq_gap->compute_at_step(step);
      }

      for (int step = nsteps; step > 0; --step) {
        dt = m_state[prob]->disc->dt(step);
        eq_gap = eq_gap_vec[step - 1];
        scaled_eq_gap = eq_gap * dt / total_time;

        J_prob += eq_gap * scaled_eq_gap;

        if (m_evaluate_gradient) {
          fail("gradient not implemented");
          /* m_eq_gap->compute_at_step_adjoint(
              step, scaled_eq_gap, grad_at_step
          ); 
...
int main(int argc, char** argv) {
  ...
  ALWAYS_ASSERT(argc == 3);
  {
    ...
    EGM_Objective objective(yaml_input, eval_grad);
    objective.evaluate();
  }
  finalize();
}
