...
class EUCLID_Objective {
  public:
    EUCLID_Objective(std::string const& input_file, bool evaluate_gradient);
    void evaluate();
  private:
...
EUCLID_Objective::EUCLID_Objective(std::string const& input_file, bool evaluate_gradient) {
...
void EUCLID_Objective::setup_opt_params(ParameterList const& inverse_params) {
...
int main(int argc, char** argv) {
  ...
  ALWAYS_ASSERT(argc == 3);
  {
    ...
    EUCLID_Objective objective(yaml_input, eval_grad);
    objective.evaluate();
  }
  finalize();
}
